...
class Unit3Dpy(torch.nn.Module):
    def __init__(
        ...
        if padding == "SAME":
            ...
            if stride[0] > 1:
                padding_shapes = [
                    get_padding_shape(kernel_size, stride, mod) for mod in range(stride[0])
                ]
            else:
...
def load_i3d_model(device, modality="rgb"):
    assert modality in ["rgb", "flow"], "Modality should be in [rgb|flow]"
    i3d = I3D(400, modality=modality)
    i3d.eval()
    current_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = "model/model_rgb.pth" if modality == "rgb" else "model/model_flow.pth"
    i3d_path = os.path.join(current_dir, model_path)
