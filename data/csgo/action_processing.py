...
@dataclass
class ReverseCSGOAction(CSGOAction):
    reverse_l_click: bool
    reverse_r_click: bool

    def __post_init__(self) -> None:
        super().__post_init__()
        if self.l_click:
            self.reverse_l_click = False
        if self.r_click:
            self.reverse_r_click = False
...
def print_csgo_action(action: CSGOAction) -> tuple[str]:
    ...
    clicks = (
        "L" * action.l_click + " + " * (action.l_click and action.r_click) + "R" * action.r_click
    )
...
N_CLICKS_WITH_REVERSE = 4  # number of mouse buttons, left, right, reverse left, reverse right
...
def decode_csgo_action(y_preds: torch.Tensor) -> ReverseCSGOAction:
...
def encode_reverse_csgo_action(
    csgo_action: ReverseCSGOAction, device: torch.device
) -> torch.Tensor:
    # mouse_x = csgo_action.mouse_x
    # mouse_y = csgo_action.mouse_y

    keys_pressed_onehot = np.zeros(N_KEYS_WITH_REVERSE)
    mouse_x_onehot = np.zeros(N_MOUSE_X)
    mouse_y_onehot = np.zeros(N_MOUSE_Y)
    l_click_onehot = np.zeros(1)
    r_click_onehot = np.zeros(1)
    reverse_l_click_onehot = np.zeros(1)
    reverse_r_click_onehot = np.zeros(1)
    ...
    for key in csgo_action.key_names:
        if key == "w":
            ...
            keys_pressed_onehot[10] = 1
        elif key == "reverse space":
            keys_pressed_onehot[11] = 1
        elif key == "reverse left ctrl":
            keys_pressed_onehot[12] = 1
        elif key == "reverse left shift":
            keys_pressed_onehot[13] = 1
        elif key == "reverse 1":
            keys_pressed_onehot[14] = 1
        elif key == "reverse 2":
            keys_pressed_onehot[15] = 1
        elif key == "reverse 3":
            keys_pressed_onehot[16] = 1
        elif key == "reverse r":
            keys_pressed_onehot[17] = 1

    l_click_onehot[0] = int(csgo_action.l_click)
    r_click_onehot[0] = int(csgo_action.r_click)
    reverse_l_click_onehot[0] = int(csgo_action.reverse_l_click)
    reverse_r_click_onehot[0] = int(csgo_action.reverse_r_click)
    ...
    return torch.tensor(
        np.concatenate(
            (
                ...
                l_click_onehot,
                r_click_onehot,
                reverse_l_click_onehot,
                reverse_r_click_onehot,
                mouse_x_onehot,
                mouse_y_onehot,
            ),
        ),
...
def decode_reverse_csgo_action(y_preds: torch.Tensor) -> CSGOAction:
    y_preds = y_preds.squeeze()
    keys_pred = y_preds[0:N_KEYS_WITH_REVERSE]
    l_click_pred = y_preds[N_KEYS_WITH_REVERSE : N_KEYS_WITH_REVERSE + 1]
    r_click_pred = y_preds[N_KEYS_WITH_REVERSE + 1 : N_KEYS_WITH_REVERSE + N_CLICKS]
    reverse_l_click_pred = y_preds[
        N_KEYS_WITH_REVERSE + N_CLICKS : N_KEYS_WITH_REVERSE + N_CLICKS + 1
    ]
    reverse_r_click_pred = y_preds[
        N_KEYS_WITH_REVERSE + N_CLICKS + 1 : N_KEYS_WITH_REVERSE + N_CLICKS_WITH_REVERSE
    ]
    # mouse_x_pred = y_preds[N_KEYS + N_CLICKS : N_KEYS + N_CLICKS + N_MOUSE_X]
    mouse_x_pred = y_preds[
        N_KEYS_WITH_REVERSE + N_CLICKS_WITH_REVERSE : N_KEYS_WITH_REVERSE
        + N_CLICKS_WITH_REVERSE
        + N_MOUSE_X
    ]
    mouse_y_pred = y_preds[
        N_KEYS_WITH_REVERSE + N_CLICKS_WITH_REVERSE + N_MOUSE_X : N_KEYS_WITH_REVERSE
        + N_CLICKS_WITH_REVERSE
        + N_MOUSE_X
        + N_MOUSE_Y
    ]
    ...
    if keys_pressed_onehot[10] == 1:
        keys_pressed.append("r")
    if keys_pressed_onehot[11] == 1:
        keys_pressed.append("reverse space")
    if keys_pressed_onehot[12] == 1:
        keys_pressed.append("reverse left ctrl")
    if keys_pressed_onehot[13] == 1:
        keys_pressed.append("reverse left shift")
    if keys_pressed_onehot[14] == 1:
        keys_pressed.append("reverse 1")
    if keys_pressed_onehot[15] == 1:
        keys_pressed.append("reverse 2")
    if keys_pressed_onehot[16] == 1:
        keys_pressed.append("reverse 3")
    if keys_pressed_onehot[17] == 1:
        keys_pressed.append("reverse r")

    l_click = int(np.round(l_click_pred))
    r_click = int(np.round(r_click_pred))
    reverse_l_click = int(np.round(reverse_l_click_pred))
    reverse_r_click = int(np.round(reverse_r_click_pred))
    ...
    return ReverseCSGOAction(
        ...
        bool(l_click),
        bool(r_click),
        bool(reverse_l_click),
        bool(reverse_r_click),
    )
