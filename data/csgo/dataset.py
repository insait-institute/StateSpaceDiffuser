...
from .utils import StateDictMixin, make_segment

log = getLogger("data_csgo", "cyan")
...
class CSGOHdf5Dataset(StateDictMixin, TorchDataset):
    ...
    def __getitem__(self, segment_id: SegmentId) -> Segment:
        assert segment_id.start < self._length_one_episode
        assert segment_id.stop > 0
        assert segment_id.start < segment_id.stop
        ...
        def pad(x: torch.Tensor) -> torch.Tensor:
            right = (
                F.pad(x, [0 for _ in range(2 * x.ndim - 1)] + [pad_len_right])
                if pad_len_right > 0
                else x
            )
            return (
                F.pad(right, [0 for _ in range(2 * x.ndim - 2)] + [pad_len_left, 0])
                if pad_len_left > 0
                else right
            )
...
class DiamondCSGODataset(StateDictMixin, TorchDataset):
    def __init__(
        self,
        directory: Path,
        dataset_full_res: TorchDataset | None,
        name: str | None = None,
    ...
    def __getitem__(self, segment_id: SegmentId) -> Segment:
        ...
        if self._dataset_full_res is not None:
            segment_id_full_res = SegmentId(
                episode.info["original_file_id"],
                segment_id.start,
                segment_id.stop,
            )
            segment.info["full_res"] = self._dataset_full_res[segment_id_full_res].obs
            # print(segment.info["full_res"].shape)
    ...
    def add_episode(self, episode: Episode, *, episode_id: int | None = None) -> int:
        self.assert_not_static()
...
class DatasetHandler(TorchDataset):
    def __init__(
        self,
        seq_len: int,
        step_size: int,
        ...
        for episode_id in range(self.dataset.num_episodes):
            episode_ids = [
                SegmentId(episode_id, i, i + self.seq_len)
                for i in range(0, self.dataset.lengths[episode_id] - self.seq_len, self.step_size)
            ]
            self.dataset_ids.extend(episode_ids)

    def __len__(self) -> int:
        return len(self.dataset_ids)

    def __getitem__(self, _id: int) -> dict[str, Any]:
        segment: Segment = self.dataset[self.dataset_ids[_id]]
        return segment
